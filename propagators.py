...
def prop_FC(csp, newVar=None):
    ...
    # Choose which constraints to check
    if newVar is None:
    ...
    for con in constraints:
        # Only forward check constraints with exactly one unassigned variable
        ...
        for var in con.get_scope():
            for val in var.cur_domain():
                if not con.check_var_val(var, val):
                    ...
                    # Add related constraints back to queue
                    for other_con in csp.get_cons_with_var(var):
                        if other_con not in gac_queue:
                            gac_queue.append(other_con)
